...
class myWidget(QMainWindow):
	...
	def showDialog(self):
		...
		f = open(fname, 'w')
		f.write(self.wnd.textEdit.toPlainText()) ##+ textedit_2
		f.close()
		#
		# with f:
		#     data = f.save()
	...
	def run(self):
		...
		while r_vnutr <= r_vnutr_max:
			self.calc_part(self.wnd.textEdit, r_vnutr, None)
			r_vnutr += r_vnutr_d
				...
				while theta2 <= theta2_max:
					...
					while pi0 <= pi0_max:
						...
						if r_vnesh:
							...
							s = 'результат при: r2 = {}; piks = {}; theta2 = {}; pi0 = {}; r_vnesh = {}'.format(r2_str, piks_str, theta2_str, pi0_str, r_vnesh_str)
						else:
							r_vnutr_str = round(r_vnutr, 3)
							s = 'результат при: r2 = {}; piks = {}; theta2 = {}; pi0 = {}; r_vnutr = {}'.format(r2_str, piks_str, theta2_str, pi0_str, r_vnutr_str)
						textEdit.append(s)
